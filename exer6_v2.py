...
def creategrid():
    ...
    if current_value == '>' and temp:
        ...
        "studemail":{"$regex": "^"+filter_mailEntry.get()}, "studcourse":{"$regex": "^"+filter_courseEntry.get()}}},
                                   {"$lookup":{"from":"subjects","localField":"enrolled.subjid","foreignField":"subjid","as":"enrolled2"}},
                                   {"$unwind": {"path":"$enrolled2", "preserveNullAndEmptyArrays":True}},
                                   {"$group":{"_id":"$_id","studid":{"$first":"$studid"},"studname":{"$first":"$studname"},
                                    "studemail": {"$first": "$studemail"},  
                                    "studcourse": {"$first": "$studcourse"},  
                                    "totunits":{"$sum": {"$toInt":"$enrolled2.subjunits"}}}},{"$sort": {"studid": 1}}])
...
def creategrid2():
    ...
    subject = mycol.aggregate([{"$match":{"studid":int(studid.get())}},
                               {"$lookup":{"from":"subjects","localField":"enrolled.subjid","foreignField":"subjid","as":"enrolled2"}},
                               {"$unwind": "$enrolled2"},
                               {"$project": {"subjid":"$enrolled2.subjid", "subjcode":"$enrolled2.subjcode", "subjdesc":"$enrolled2.subjdesc","subjunits":"$enrolled2.subjunits", "subjsched":"$enrolled2.subjsched"}}])
...
#exer 6
def get_last_row(grid_frame):
    last_row = 0
    for slave in grid_frame.grid_slaves():
        info = slave.grid_info()
        if info['row'] > last_row:
            last_row = info['row']
    return last_row
...
def delete_grade_grid():
    ...
    for label in window.grid_slaves():
        if (int(label.grid_info()["row"]) > last_row - 3) and (int(label.grid_info()["column"]) >= 3) and (int(label.grid_info()["column"]) < 10):
            label.grid_forget()
...
def create_grade_grid():
    ...
    label = tk.Label(window, text="Midterm", width=13, height= 1, bg="yellow", anchor="center")
    label.grid(column = 5, row = last_row + 2)

    label = tk.Label(window, text="Prefinal", width=13, height= 1, bg="yellow", anchor="center")
    label.grid(column = 6, row = last_row + 2)
